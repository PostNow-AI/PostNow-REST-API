#!/usr/bin/env python3
"""
Script para enviar o segundo email da campanha de reativação.
Envia para usuários que receberam Email 1 e ainda não reativaram.

Assunto: O Plano Legado: um presente para quem começou com a gente
Timing: Dia 3 (3 dias após Email 1)

Uso:
    python scripts/reactivation_email_2.py [--dry-run] [--user-id=<uuid>]
"""
import asyncio
import os
import sys
...
async def get_target_users():
    """
    Retorna usuários elegíveis para o email 2 de reativação.
    
    Critérios:
    - Receberam Email 1
    - NÃO reativaram desde então
    - NÃO receberam Email 2 antes
    """
    cutoff_date = timezone.make_aware(datetime(2025, 12, 25, 23, 59, 59))

    # Usuários que receberam Email 1
    users_with_email_1 = await sync_to_async(lambda: ReactivationEmail.objects.filter(
    ...
    # Usuários que já receberam Email 2
    ...
    users_with_email_1_list = await sync_to_async(lambda: list(users_with_email_1))()
    ...
    print(f'Users with expired subs: {len(users_with_email_1_list)}')
    print(f'Users who reactivated: {len(users_who_reactivated_list)}')
    print(f'Users already sent: {len(users_already_sent_list)}')

    # Filtra usuários elegíveis
    target_users = await sync_to_async(lambda: User.objects.filter(
        id__in=users_with_email_1
    ).exclude(
...
async def send_reactivation_email_2(user, dry_run=False):
    """
    Envia o segundo email de reativação para um usuário.
    ...
    try:
        ...
        if existing and existing.sent_at:
            return False, f"Email 2 já enviado em {existing.sent_at}"
        ...
        # Renderiza template
        html_content = await sync_to_async(render_to_string)('reactivation/email_2.html', context)
...
async def main_async(dry_run=False, user_id=None):
    ...
    print("=" * 70)
    print("📧 CAMPANHA DE REATIVAÇÃO - EMAIL 2 (DIA 3)")
    ...
    for user in users_list:
        success, message = await send_reactivation_email_2(user, dry_run=dry_run)
